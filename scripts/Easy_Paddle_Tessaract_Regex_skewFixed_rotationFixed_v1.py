...
output_file = r'C:\Users\hp\Downloads\amat_final_work\Data-Extraction-from-PDF\corrected5_output.pdf'
...
def rotate_image(pil_image, angle):
    ...
    if angle == 0:
        return pil_image
    return pil_image.rotate(-angle, expand=True)  # Negative for correction
...
def paddle_ocr_extract(image):
    ...
    try:
        ...
        # Run OCR prediction - Use the correct method
        try:
            # Newer versions use .ocr() method
        ...
        if result and len(result) > 0:
            # New version returns a list of results per image
...
def process_all_pages():
    ...
    try:
        ...
        for page_num, original_image in page_images:
            ...
            # Convert to PIL for orientation detection
            pil_img = Image.fromarray(cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB))
            
            # Step 1: Correct major orientation
