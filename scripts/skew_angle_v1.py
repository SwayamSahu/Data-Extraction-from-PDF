...
pdf_file = r'C:\Users\SwayamPrakashSahu\Downloads\AMAT_TableExtractor\input_pdfs\505150_3030065986_1060_BOL (2).pdf'
output_file = r'C:\Users\SwayamPrakashSahu\Downloads\AMAT_TableExtractor\corrected4_output.pdf'
...
def detect_table_structure(words):
    ...
    Clusters words into rows and columns based on their Y and X coordinates using adaptive tolerances.
    Validates table structures with an alignment score, ensuring robustness for both table types.
    Supports both grid-based tables (where text aligns with cell boundaries) and space-separated tables (where text aligns with consistent spacing).
    """
...
def enhanced_get_ocr_score(image):
    """Enhanced OCR scoring that considers table structure"""
...
def find_best_orientation(image):
    ...
    for rotation, rotated_img in orientations.items():
        score, word_count, confidence, table_info, text_angles = enhanced_get_ocr_score(rotated_img)
...
def correct_page_orientation(image):
    ...
    best_rotation, scores = find_best_orientation(image)
...
def correct_pdf_skew(pdf_path, output_path):
    ...
    print("Starting PDF correction with enhanced table detection...")
    ...
    for page_num, original_image in page_images:
        ...
        # Correct orientation
        corrected_image, major_rotation, fine_skew, orientation_scores = correct_page_orientation(original_image)
...
if __name__ == "__main__":
    print("Enhanced PDF Orientation Correction Tool")
    ...
    try:
        ...
        print(f"{'='*60}")
        print("Enhanced PDF correction completed successfully!")
